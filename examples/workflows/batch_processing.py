...
class BatchProcessor:
            ...
            try:
                ...
                try:
                    ...
                    if json_start != -1 and json_end > json_start:
                        ...
                        result_data = {"sentiment": "unknown", "confidence": 0, "emotions": [], "explanation": response[:100]}
                except json.JSONDecodeError:
                    result_data = {"sentiment": "error", "confidence": 0, "emotions": [], "explanation": response[:100]}
                ...
                print(f"  ✅ {i}/{len(texts)} - {result_data.get('sentiment', 'unknown').upper()}")
    ...
    def process_text_summarization(self, texts: List[str], max_length=100) -> List[Dict[str, Any]]:
        ...
        for i, text in enumerate(texts, 1):
            try:
                prompt = f"""Summarize this text in exactly {max_length} words or less. 
                ...
                results.append(result)
                print(f"  ✅ {i}/{len(texts)} - {len(text.split())} → {len(response.strip().split())} words")
    ...
    def process_translation(self, texts: List[str], target_language="Spanish") -> List[Dict[str, Any]]:
    ...
    async def async_batch_process(self, texts: List[str], task_type="sentiment") -> List[Dict[str, Any]]:
        ...
        async def process_single(text, index):
            try:
                if task_type == "sentiment":
                    ...
                    prompt = f"""Summarize in 50 words: "{text}"""
                else:
        ...
        print(f"✅ Completed: {len(successful_results)} successful, {len(error_results)} errors")
    ...
    def get_stats(self):
        ...
        return {
            ...
            "success_rate": (self.processed_count - len(self.errors)) / max(self.processed_count, 1) * 100
        }
    ...
    try:
        ...
        while True:
            ...
            if choice == '1':
                ...
                results = processor.process_text_summarization(sample_texts, max_length=30)
                ...
                language = input("Target language (default: Spanish): ").strip() or "Spanish"
                ...
                results = asyncio.run(processor.async_batch_process(sample_texts[:4], "sentiment"))
                ...
                if os.path.exists(filename):
                    ...
                    task = input("Task (sentiment/summary/translation): ").strip().lower()
