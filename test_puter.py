...
def main():
    ...
    try:
        ...
        if client.login():
            ...
            print("Type 'exit' to quit, '/model <name>' to change model, '/clear' to clear chat history.")
                ...
                if user_input.lower() == 'exit':
                    ...
                    if len(parts) > 1:
                        ...
                        if client.set_model(new_model):
                            print(f"Model successfully changed to: {client.current_model}")
                        else:
                            print(f"Error: Model '{new_model}' not found. Available models: {', '.join(client.get_available_models())}")
                    else:
                        print(f"Current model: {client.current_model}. Use '/model <name>' to change.")
                    ...
                    for model in client.get_available_models():
                        print(f"- {model}")
                elif user_input.strip(): # Only call AI if user input is not empty and not a command
                    try:
